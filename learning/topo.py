...
# total pop of layer 4 of area W
...
spike_detectors = []
layers = []
for ctr in range(len(population_properties)):
...
tonotopic_excitatory = {
    ...
    'weights': {'gaussian': {'p_center': 1.0,'sigma': 10.0},
...
excitatory = {
  'connection_type': 'divergent',
  'synapse_model': 'excitatory',
  'weights': {'uniform': {'min': 0.0,'max': 0.1}}
}
...
excitatory_topological = {
  'connection_type': 'divergent',
  'synapse_model': 'excitatory',
  'weights': {'uniform': {'min': 0.0,'max': 5.0}
    },
  'kernel': {'gaussian': {'p_center': 1.0,'sigma': radius/2}}
}
...
inhibitory_layerW = {
  ...
  'weights': {'uniform': {'min': -5.0,'max': 0.0}}
}
...
inhibitory = {
  'connection_type': 'divergent',
  'synapse_model': 'inhibitory',
  'weights': {'uniform': {'min': -0.1,'max': 0.0}}
}
...
spike_id = spikes[0]+1
spike_ids = tuple([x for x in range(spike_id, spike_id+(spike_rows*spike_rows))])
